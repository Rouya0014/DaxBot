    ...
    run: async (client, interaction, config, db) => {
        ...
        if (targetUserRolePosition >= botRolePosition) {
          ...
          if (targetUser.isCommunicationDisabled()) {
            ...
            await interaction.reply({content: "<:ErrorIcon:1098685738268229754> Je ne peux pas exclure cet utilisateur car il déjà exclu", ephemeral: true});
            return;
          }
          ...
          const timeoutEmbed = new EmbedBuilder()
            .setAuthor({name: `${targetUser.user.tag} a été exclu pendant ${durationInWords}`, iconURL: `${targetUser.displayAvatarURL({size: 512, dynamic: true})}`})
            .setDescription(`<:TimeoutClockIcon:1088580887429525565> **Raison** : ${reason}`)
            .setColor('#ee2346')
          ...
          client.on("interactionCreate", async interaction => {
            ...
            if (interaction.customId === "cancel") {
              ...
              const untimeout = new EmbedBuilder()
                .setAuthor({name: `${targetUser.user.tag} n'est plus exclu` , iconURL: `${targetUser.displayAvatarURL({size: 512, dynamic: true})}`})
                .setColor('#278048')
                ...
                interaction.deferUpdate().then(() => {
                  ...
                  const unimeoutembed = new EmbedBuilder()
                  .setAuthor({name: `${targetUser.user.tag}`, iconURL: `${targetUser.displayAvatarURL({size: 512, dynamic: true})}`})
                  .setThumbnail(targetUser.displayAvatarURL({size: 512, dynamic: true}))
                  ...
                  logChannel.send({embeds: [unimeoutembed]})
                }).catch(console.error);
            }
          })
    },
