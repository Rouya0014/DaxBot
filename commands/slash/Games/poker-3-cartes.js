...
function isStraightFlush(ranks, suits) {
  ...
  const isStraight = indices.every((val, index, arr) => index === 0 || val - arr[index - 1] === 1);
...
function isThreeCardStraight(ranks, suits) {
...
function evaluateHand(hand) {
  ...
  return { evaluation, score, highCard };
}

module.exports = {
  name: "poker3",
  description: "🎰 | Jouez au Poker à 3 cartes.",
  type: 1,
  options: [
    {
      name: "mise",
  ...
  run: async (client, interaction) => {
    ...
    if (playerHandEvaluation.evaluation === 'Hauteur') {
      playerHandValue += ` (Carte haute : ${cardToString(playerHandEvaluation.highCard)})`;
    }

    let dealerHandValue = dealerHandEvaluation.evaluation;
    if (dealerHandEvaluation.evaluation === 'Hauteur') {
      dealerHandValue += ` (Carte haute : ${cardToString(dealerHandEvaluation.highCard)})`;
    }

    const embed = new EmbedBuilder()
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.displayAvatarURL({ size: 512, dynamic: true })})
      .addFields(
        {name: 'Votre main', value: `${playerHand.map(cardToString).join(' ')}\n\nValeur : ${evaluateHand(playerHand).evaluation}${evaluateHand(playerHand).evaluation === 'Hauteur' ? ` (${cardToString(evaluateHand(playerHand).highCard)})` : ''}`, inline: true},
        {name: 'Main du croupier', value: `${cardEmojis.back} ${cardEmojis.back} ${cardEmojis.back}`, inline: true}
      )
      .setColor('#5865f2')
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'fold') {
        ...
        const playEmbed = new EmbedBuilder()
          ...
          .addFields(
            {name: 'Votre main', value: `${playerHandStr}\n\nValeur : ${playerHandEvaluation.evaluation}${playerHandEvaluation.evaluation === 'Hauteur' ? ` (${cardToString(playerHandEvaluation.highCard)})` : ''}`, inline: true},
            {name: 'Main du croupier', value: `${dealerHandStr}\n\nValeur : ${dealerHandEvaluation.evaluation}${dealerHandEvaluation.evaluation === 'Hauteur' ? ` (${cardToString(dealerHandEvaluation.highCard)})` : ''}`, inline: true}
          )
          .setColor(color)
    ...
    collector.on('end', async () => {
      if (!gameEnded) {
        ...
        if (playerHandEvaluation.score > dealerHandEvaluation.score) {
          ...
          // Égalité : comparer les cartes
          const playerHighCard = getHighCard(playerHand);
          const dealerHighCard = getHighCard(dealerHand);

          const rankOrder = ['2', '3', '4', '5', '6', '7', '8', '9', '10', 'j', 'q', 'k', 'a'];
          const playerHighCardRankIndex = rankOrder.indexOf(playerHighCard.rank);
          const dealerHighCardRankIndex = rankOrder.indexOf(dealerHighCard.rank);
