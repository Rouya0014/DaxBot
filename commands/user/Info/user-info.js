...
module.exports = {
    ...
    run: async (client, interaction, config, db) => {
        ...
        let Badg = user.user.flags.toArray().map((flag) => badges[flag]).join(" ");
      ...
      .addFields(
        ...
        if (user.user.avatar && user.user.avatar.startsWith("a_") || user.premiumSinceTimestamp) {
          embed.addFields({ name: "<:FreeSubscriptionIcon:1098582543198077059> Badges", value: Badg  ? "> " + Badg + " <:SubscriberNitroIcon:1088197836832788480>" : "> <:SubscriberNitroIcon:1088197836832788480>"});
        } else {
          if (!user.user.bot) {
          embed.addFields({ name: "<:FreeSubscriptionIcon:1098582543198077059> Badges", value: Badg  ? `> ${Badg}` : "> Aucun badge"});
        } else if (user.user.bot && Badg){
          embed.addFields({ name: "<:FreeSubscriptionIcon:1098582543198077059> Badges", value: Badg ? `> ${Badg}` : "> Aucun badge"});
        } else {
